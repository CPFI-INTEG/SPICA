                  ...
                  <tbody >
                    ...
                    <?php
                       ...
                       if($UserType=="Sales Finance Assistant"){// to know if this is SALES finance
                          $sql="select distinct dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke from tip.V_PARTIAL_BALANCE where isstatuscomplete='Completed' and actual_month in  (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] )";                            
                          //and DATEPART(month, or_Date)=DATEPART(month, getdate())-2
                        }elseif($UserType=="District Business Development Manager"){ 
                          $sql="select distinct IsStatusSubmitted,name,dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke from tip.V_PARTIAL_BALANCE where isstatussubmitted='Submitted' and actual_month in  (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] ) and [DBDM Name]='".$DBDM_Name."'";                                                           
                        
                        }elseif($UserType=="Credit and Collection Supervisor"){ // credit and collection to!
                          $sql="select distinct dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke from tip.V_PARTIAL_BALANCE where isstatussubmitted='Approved' and actual_month in  (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] )";                            
                        }

                        else{
                          $sql="select dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke from tip.V_PARTIAL_BALANCE  where bdm_username='".$parentCode."' and IsStatusSubmitted='Submitted' and bp_type_cd='". $channel ."'   AND  dim5_Cd in (". $BusinessUnit .") and actual_month in (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] where actual_year in (select max(actual_year) from [TIP].[V_PARTIAL_BALANCE] ) )";
