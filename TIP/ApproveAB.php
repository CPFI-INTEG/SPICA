                  ...
                  <tbody >
                    ...
                    <?php
                        ...
                        if($UserType=="Sales Finance Assistant" or $UserType=="Sales Controller"){// to know if this is SALES finance
                          $sql="select document_no,dim5_Desc,ID,division,target_Company,bp_parent_Cd,bp_parent_name,budget_desc,transaction_Amount,dim4_Cd ,isstatus,ispoke,IsStatusSubmitted from tip.Vw_BDMUserTagging where IsStatuscomplete='Completed' and id in (select dataid from tip.tir_Shouldbe where PromoID='Add Back')";
                          ...
                          $sql="select document_no,dim5_Desc,ID,division,target_Company,bp_parent_Cd,bp_parent_name,budget_desc,transaction_Amount,dim4_Cd ,isstatus,ispoke,IsStatusSubmitted from tip.Vw_BDMUserTagging where IsStatuscomplete='Completed' and IsStatusSubmitted not in('Approved','Disapproved')  and  id in (select dataid from tip.tir_Shouldbe where PromoID='Add Back')";                           
                          //and actual_month in (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] where actual_year in (select max(actual_year) from [TIP].[V_PARTIAL_BALANCE] ) ) 
                        }elseif($UserType=="District Business Development Manager"){ 
                          $sql="select document_no,dim5_Desc,ID,division,target_Company,bp_parent_Cd,bp_parent_name,budget_desc,transaction_Amount,dim4_Cd ,isstatus,ispoke,IsStatusSubmitted from tip.Vw_BDMUserTagging where [DBDM Name]='".$DBDM_Name."'  and  id in (select dataid from tip.tir_Shouldbe where PromoID='Add Back')";
                        }else{
                          $sql="select document_no,dim5_Desc,ID,division,target_Company,bp_parent_Cd,bp_parent_name,budget_desc,transaction_Amount,dim4_Cd ,isstatus,ispoke,IsStatusSubmitted from tip.Vw_BDMUserTagging where bdm_username='".$parentCode."' and company_Cd in (". $BusinessUnit .")  and  id in (select dataid from tip.tir_Shouldbe where PromoID='Add Back')";
                        }
