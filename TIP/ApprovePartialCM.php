...
<div class="wrapper">
  ...
  <div class="content-wrapper">
   ...
   <section class="content">
      <h1>APPROVED DATA (Credit Memo)</h1>
                    ...
                    <?php
                       ...
                       include("model/dbcon.php");
                        ...
                        elseif($UserType=="Credit and Collection Supervisor" or $UserType=="Credit Analyst"){ // credit and collection to!
                          $sql="select distinct dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke from tip.V_PARTIAL_BALANCE where isstatussubmitted='Approved' and actual_month in  (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] ) and id in (select dataid from tip.pb_Shouldbe where Description='Credit Memo')";                            
                        }
                        else{
                          $sql="select dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke from tip.V_PARTIAL_BALANCE  where bdm_username='".$parentCode."'  AND  dim5_Cd in (". $BusinessUnit .") and IsStatusSubmitted='Submitted' and bp_type_cd='". $channel ."' and actual_month in  (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] ) and id in (select dataid from tip.pb_Shouldbe where Description='Credit Memo')";                            
