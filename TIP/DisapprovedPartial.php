                  ...
                  <tbody >
                    ...
                    <?php
                        ...
                        if($UserType=="Sales Finance Assistant" or $UserType=="Sales Controller"){// to know if this is SALES finance
                            $sql="select distinct dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke  from tip.V_PARTIAL_BALANCE where IsStatusSubmitted='Disapproved'";
                        }elseif($UserType=="Credit and Collection Supervisor" ){
                          $sql="select distinct dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke  from tip.V_PARTIAL_BALANCE where  IsStatusSubmitted='Disapproved'";
                        }
                        elseif($UserType=="Credit Analyst"){
                          
                          $sql="select distinct dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke  from tip.V_PARTIAL_BALANCE where  IsStatusSubmitted='Disapproved' and id in (select dataid from tip.PB_ShouldBE where description='Credit Memo') and actual_month in  (select max(actual_month) from [TIP].[V_PARTIAL_BALANCE] ) and [Analyst Username]='".$parentCode."' ";                           
                        }
                        
                        else{
                          $sql="select dim5_Desc_Division,Company_CD,[PARENT_NAME] as parent_name ,DIM4_Cd,DISTRICT,ischeck,isstatus,isstatussubmitted,isstatuscomplete,ID,ispoke  from tip.V_PARTIAL_BALANCE where bdm_username='".$parentCode."' and IsStatusSubmitted='Submitted' and  bp_type_Cd ='". $channel ."' AND  dim5_Cd in (". $BusinessUnit .")";
                        }
                         ...
                         while($row=sqlsrv_fetch_array($stmt,SQLSRV_FETCH_ASSOC)){
                            $strDisplay.="<tr>";
                                if($UserType=="Sales Finance Assistant" or $UserType=="Sales Controller"){ // to know if this is SALES ADMIN
                                  ...
                                  $strDisplay.="<td>
                                      <a href='ViewDisapprovedPartial.php?ID=$row[ID]' class='btn medium bg-red tooltip-button' data-placement='top'>
                                          &nbsp;&nbsp;&nbsp;View&nbsp;&nbsp;&nbsp;
                                      </a>                                   
                                  </td>";   
